...
[![Version](https://img.shields.io/badge/version-5.1.12-blue.svg)](https://github.com/Dicklesworthstone/ultimate_bug_scanner)
...
| Security & Taint | **3** – Lightweight taint engine + XSS/prototype/window.open/JSX target blank security heuristics (cat.7) | **2** – Eval/exec/SQL string heuristics | **2** – HTTP/crypto/command checks | **1** – Limited dedicated security (mostly UB) | **2** – Security category (cat.8) | **3** – SQL concat, `Runtime.exec`, SSL/crypto checks | **2** – Rails mass-assignment, shell/eval warnings | **2** – weak crypto, TLS bypass, hardcoded secrets, ASP.NET web hazards |
