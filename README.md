...
[![Version](https://img.shields.io/badge/version-5.1.76-blue.svg)](https://github.com/Dicklesworthstone/ultimate_bug_scanner)
...
- **Rust** – A Python helper inspects `if let Some/Ok` guard clauses and flags subsequent `.unwrap()`/`.expect()` calls outside of exiting blocks. The Rust security pass also catches archive member paths flowing into extraction destinations without `enclosed_name()`/`unpack_in()`-style containment. Fixtures and manifest cases keep these regressions tested.
