...
| Python | `test-suite/python/buggy/`, `test-suite/python/security/` | `test-suite/python/clean/`, `test-suite/python/security/` | Async/security/resource fixtures + mutable defaults, shell injection, command execution dataflow, subprocess timeout coverage, SQL/NoSQL injection sinks, request-controlled regex patterns, template injection sources, response header injection, email header injection, LDAP injection, archive traversal, open redirects, host header poisoning, SSRF, outbound HTTP timeouts, path traversal/upload filename saves, JWT verification, credentialed CORS, session cookie flags, CSRF disables, template autoescape disables, safe-HTML XSS sinks, mass-assignment sinks, unsafe deserialization loaders, weak password hashers, ECB/static-IV crypto misuse, non-constant-time secret comparisons, security-sensitive asserts, world-writable file modes, debug/host configuration, unsafe XML parsing, insecure security randomness, disabled TLS verification |
| Go | `test-suite/golang/buggy/`, `test-suite/golang/security/` | `test-suite/golang/clean/`, `test-suite/golang/security/` | Concurrency, HTTP safety, context leaks, ticker/perf issues, request path traversal, request-derived outbound URL/SSRF, archive extraction |
| C/C++ | `test-suite/cpp/buggy/`, `test-suite/cpp/security/` | `test-suite/cpp/clean/`, `test-suite/cpp/security/` | RAII vs leaks, unsafe `strcpy`, request path traversal, archive extraction, overflow/memory hygiene |
...
| Java | `test-suite/java/buggy/`, `test-suite/java/security/` | `test-suite/java/clean/`, `test-suite/java/security/` | Executor leaks, blocking I/O, SQL/command injection, request path traversal, request-derived outbound URL/SSRF, archive extraction |
| Kotlin (Java-family scanner) | `test-suite/kotlin/security_buggy/`, `test-suite/kotlin/path_traversal_buggy/`, `test-suite/kotlin/archive_extraction_buggy/`, `test-suite/kotlin/type_narrowing/buggy/` | `test-suite/kotlin/security_clean/`, `test-suite/kotlin/path_traversal_clean/`, `test-suite/kotlin/archive_extraction_clean/`, `test-suite/kotlin/type_narrowing/clean/` | ProcessBuilder shell injection, request path traversal, archive extraction path traversal, nullable guard fallthrough |
| Ruby | `test-suite/ruby/buggy/`, `test-suite/ruby/path_traversal_buggy/`, `test-suite/ruby/archive_extraction_buggy/`, `test-suite/ruby/ssrf_buggy/` | `test-suite/ruby/clean/`, `test-suite/ruby/path_traversal_clean/`, `test-suite/ruby/archive_extraction_clean/`, `test-suite/ruby/ssrf_clean/` | eval/YAML problems, thread leaks, file cleanup, request path traversal, request-derived outbound URL/SSRF, archive extraction |
| Swift | `test-suite/swift/buggy/`, `test-suite/swift/archive_extraction_buggy/`, `test-suite/swift/path_traversal_buggy/`, `test-suite/swift/ssrf_buggy/`, `test-suite/swift/type_narrowing/buggy/` | `test-suite/swift/clean/`, `test-suite/swift/archive_extraction_clean/`, `test-suite/swift/path_traversal_clean/`, `test-suite/swift/ssrf_clean/`, `test-suite/swift/type_narrowing/clean/` | Shell-backed process execution, request path traversal, request-derived outbound URL/SSRF, archive extraction, optional guard fallthrough |
| C# | `test-suite/csharp/buggy/`, `test-suite/csharp/security/` | `test-suite/csharp/clean/`, `test-suite/csharp/security/` | Task blocking, weak crypto, request path traversal, request-derived outbound URL/SSRF, archive extraction, `throw ex`, `TryParse` vs `Parse`, null/type narrowing fallthrough, helper-backed resource lifecycle, unobserved `Task.Run`/`StartNew` handles |
| Elixir | `test-suite/elixir/buggy/` | `test-suite/elixir/clean/` | Shell-backed command execution, request path traversal, request-derived outbound URL/SSRF, archive extraction |
...
| `java-request-path-traversal-buggy` | `test-suite/java/security/path_traversal_buggy.java` | Request parameters, path info, and upload filenames flow into Java file read/write/delete sinks without containment checks. |
| `java-request-path-traversal-clean` | `test-suite/java/security/path_traversal_clean.java` | Uses `toRealPath().normalize()` plus `startsWith` containment or `Path.getFileName()` before file sinks. |
| `java-ssrf-buggy` | `test-suite/java/security/SsrfBuggy.java` | Request parameters and headers flow into JDK `HttpClient`, `RestTemplate`, and `URL.openStream` without scheme and host allow-list validation. |
| `java-ssrf-clean` | `test-suite/java/security/SsrfClean.java` | Uses named safe outbound URL helpers with URL parsing and host allow-list checks before client request sinks. |
...
| `kotlin-request-path-traversal-buggy` | `test-suite/kotlin/path_traversal_buggy` | Kotlin Ktor-style params, request paths, and upload filenames flow into file read/write/serve/delete sinks without basename or root containment checks. |
| `kotlin-request-path-traversal-clean` | `test-suite/kotlin/path_traversal_clean` | Kotlin fixtures that use safe root containment or `Path.fileName` reduction before touching file sinks. |
